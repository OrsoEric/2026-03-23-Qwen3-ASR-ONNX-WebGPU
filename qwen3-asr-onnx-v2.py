...
class SimpleTokenizer:
    """Minimal tokenizer using tokenizers library (or local tokenizer.json)."""
...
class OnnxAsrPipeline:
    ...
    def __init__(self, onnx_dir: str = "onnx_models", num_threads: int = 0,
        ...
        if quantize == "int8":
            if (onnx_path / "onnx_models" / "decoder_init.int8.onnx").exists():
                s_decoder_init_name = "decoder_init.int8.onnx"
                s_decoder_step_path = "decoder_step.int8.onnx"
                print(f"Loading ONNX models (decoder: INT8)...")
            else:
                print("ERROR: INT8 does not exist")
                return True
        else:
            s_decoder_init_name = "decoder_init.onnx"
            s_decoder_step_path = "decoder_step.onnx"
            print(f"Loading ONNX models (decoder: FP32)...")


        # encoder_conv
        s_encoder_conv_path = Path(onnx_path / "onnx_models" / "encoder_conv.onnx")
        print(f"Loading encoder_conv ({s_encoder_conv_path.stat().st_size / 1e6:.0f} MB)...")
        if not s_encoder_conv_path.exists():
            raise Exception(f"cannot find {s_encoder_conv_path}")
        self.encoder_conv = ort.InferenceSession(
            s_encoder_conv_path, sess_opts,
            providers=[C_S_EXECUTION_PROVIDER]
        )

        # encoder_transformer
        s_encoder_transformer_path = Path(onnx_path / "onnx_models" / "encoder_transformer.onnx")
        print(f"Loading encoder_transformer ({s_encoder_transformer_path.stat().st_size / 1e6:.0f} MB)...")
        if not s_encoder_transformer_path.exists():
            raise Exception(f"cannot find {s_encoder_transformer_path}")
        self.encoder_transformer = ort.InferenceSession(
            s_encoder_transformer_path, sess_opts,
            providers=[C_S_EXECUTION_PROVIDER]
        )

        # decoder_init
        s_decoder_init_path = Path(onnx_path / "onnx_models" / s_decoder_init_name)
        print(f"Loading decoder_init ({s_decoder_init_path.stat().st_size / 1e6:.0f} MB)...")
        if not s_decoder_init_path.exists():
            raise Exception(f"cannot find {s_decoder_init_path}")
        self.decoder_init = ort.InferenceSession(
            s_decoder_init_path, sess_opts,
            providers=[C_S_EXECUTION_PROVIDER]
        )

        # decoder_step
        s_decoder_step_path = Path(onnx_path / "onnx_models" / s_decoder_step_path)
        print(f"Loading decoder_step ({s_decoder_step_path.stat().st_size / 1e6:.0f} MB)...")
        if not s_decoder_step_path.exists():
            raise Exception(f"cannot find {s_decoder_step_path}")
        self.decoder_step = ort.InferenceSession(
            s_decoder_step_path, sess_opts,
            providers=[C_S_EXECUTION_PROVIDER]
        )

        # Load embedding matrix
        embed_path = onnx_path / "onnx_models" / "embed_tokens.bin"
        ...
        # Tokenizer: Look for tokenizer.json in the root directory
        tokenizer_path = onnx_path / "tokenizer.json"
        print(f"Loading tokenizer at: {tokenizer_path} | Size: ({tokenizer_path.stat().st_size / 1e6:.0f} MB)...")
        if not tokenizer_path.exists():
            raise FileNotFoundError(f"Tokenizer not found at {tokenizer_path}")
        self.tokenizer = SimpleTokenizer(str(tokenizer_path))
...
def main():
    ...
    cl_pipeline = OnnxAsrPipeline(onnx_dir=args.onnx_dir, num_threads=args.threads,
    ...
    for audio_path in args.audio:
        ...
        if args.json:
            ...
            print(f"  {result['text']}")
            print(f"  Encoder: {t['encoder_s']:.3f}s | Prefill: {t['prefill_s']:.3f}s | Decode: {t['decode_s']:.3f}s | Tokens: {t['tokens_generated']} | TPS {t['tokens_generated']/t['decode_s']}")
