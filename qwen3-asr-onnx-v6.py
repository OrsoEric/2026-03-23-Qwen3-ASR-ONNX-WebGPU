...
- Uses on-the-fly ArgMax to avoid creating patched ONNX files
"""

#python qwen3-asr-onnx-v6.py "D:\Data\Project\Project-LLM\Audio Samples\SAMPLE-Ranni-18s.wav" "D:\Data\Project\Project-LLM\Audio Samples\SAMPLE-british-man-7s.wav"  --onnx-dir "D:\LLM_Models\Qwen\Qwen3-ASR-0.6B-ONNX-CPU"
...
class Pipeline:
    ...
    def transcribe(self, path, language=None):
        ...
        # Compute first token on-the-fly
        ...
        # Pre-allocate GPU buffers (still using NumPy arrays; ORT handles device upload)
        token_embed = np.zeros((1,1,HIDDEN_SIZE), dtype=np.float32)
        pos_buf = np.zeros((1,1), dtype=np.int64)
        ...
        # keep past_keys and past_values on GPU
        for _ in range(C_N_MAX_TOKENS):
            if next_token in (IM_END_ID, ENDOFTEXT_ID):
                break

            # overwrite embedding and position
            token_embed[0,0] = self.ann_embed[next_token]
            pos_buf[0,0] = cur

            io = self.cl_onnx_decoder_step.io_binding()
            io.bind_ortvalue_input("input_embeds", token_embed)
            io.bind_ortvalue_input("position_ids", pos_buf)
            io.bind_ortvalue_input("past_keys", k)
            io.bind_ortvalue_input("past_values", v)

            # bind outputs, keep them on GPU
            io.bind_output("logits")
            io.bind_output("present_keys")
            io.bind_output("present_values")

            # run step entirely on WebGPU
            self.cl_onnx_decoder_step.run_with_iobinding(io)

            # get outputs back on GPU; do not copy to CPU yet
            # in ort >=1.15, outputs can stay as GPU OrtValues
            outputs = io.copy_outputs_to_numpy()  # only minimal copy if needed
            logits_gpu, k, v = outputs  # logits still on GPU if provider supports

            # compute next token directly on GPU if possible (otherwise minimal CPU copy)
            next_token = int(np.argmax(logits_gpu[0, -1]))
            generated.append(next_token)
            cur += 1

        # Only now, after all tokens are generated, copy embeddings or token IDs to CPU
        final_tokens = np.array(generated)
        text = self.cl_tokenizer.decode(final_tokens)
